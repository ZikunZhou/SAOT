...
visual_root_path = '/home/zikun/work/tracking/KPT/analyze/'
...
class GLSE(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        test_x, test_skfeat = self.get_classification_features(backbone_feat)
    ...
    def localize_target(self, scores, sample_pos, sample_scales):
        ...
        if preprocess_method == 'none':# actually none
            pass
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales):
        ...
        returns:
            translation_vec1 (Tensor) - translation in the original image scale
        """
    ...
    def init_state_estimator(self, se_feat, target_box):
        return self.net.state_estimator.init(se_feat, target_box)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        # Extract augmented image patches im_patches: torch.Tensor, shape=[13, 3, 288, 288]
    ...
    def get_iounet_box(self, pos, sz, sample_pos, sample_scale):
        ...
        Generates a box in the cropped image sample reference frame, in the format used by the IoUNet (x1, y1, w, h)."""
        box_center = (pos - sample_pos) / sample_scale + (self.img_sample_sz - 1) / 2
        box_sz = sz / sample_scale
        target_ul = box_center - (box_sz - 1) / 2
        return torch.cat([target_ul.flip((0,)), box_sz.flip((0,))])

    def get_subsearch_window(self, pos, sample_pos, sample_scale, subsearch_sz):
        """
        pos and sample_pos are in original image coordinates, coordinate_defination [y, x]
        returns:
            subsearch_window_coord (Tensor, cpu) - coordinate_defination [x1, y1, w, h]
            window_lt (Tensor, cpu) - coordinate_defination [y, x]
            predicted_center (Tensor, cpu) - coordinate_defination [yc, xc]
        """

        window_center = (pos - sample_pos) / sample_scale + (self.img_sample_sz - 1)/2

        y1, x1 = window_center - (subsearch_sz - 1)/2
        y2, x2 = window_center + (subsearch_sz - 1)/2

        shift_y = float(max(0, -y1) + min(0, self.img_sample_sz[0] - 1 - y2))
        y1 += shift_y
        y2 += shift_y
        shift_x = float(max(0, -x1) + min(0, self.img_sample_sz[1] - 1 - x2))
        x1 += shift_x
        x2 += shift_x

        subsearch_window_coord = torch.tensor([[x1, y1, x2-x1+1, y2-y1+1]])
        window_lt = torch.tensor([y1, x1])
    ...
    def init_glse_net(self, backbone_feat):
        ...
        if self.params.iounet_augmentation:# False, if True: use the bboxes of all feasible augmented inital images
            for T in self.transforms:
        ...
        else:# use the bbox from the first augmentation
        ...
        if target_boxes.shape[0] == 1:
            ...
            se_feat = self.adjust_skfeat(backbone_feat[0:target_boxes.shape[0],...])

        self.net.state_estimator.eval()
        # Fow now, we only use the sample generated by the first augmentation as the template
        self.init_search_feat, self.template = self.init_state_estimator(se_feat, target_boxes)


    def adjust_skfeat(self, skfeat):
        """
        args:
            skfeat (Tensor) - shape = [num_sample, channel, height, width]
            self.target_filter (Tensor) - shape = [1, channel, 4, 4]
            self.downsample_param (Tensor) - shape = [1, channel, 1, 1]
        """
        channel_importance = F.adaptive_avg_pool2d(self.target_filter.clone().detach(), (1,1)) / self.downsample_param
        normed_channel_importance = F.sigmoid(channel_importance)
        adjusted_skfeat = skfeat * normed_channel_importance
        return adjusted_skfeat

    def init_classifier(self, init_backbone_feat):
        # Get classification features
        x, skfuse_feat = self.get_classification_features(init_backbone_feat)#
        x = x.detach()
        ...
        # Get the pooled parameter of the downsample layer and compute the channel importance
        downsample_param = self.net.classifier.feature_extractor.downsample_layer.weight.data.clone().detach()
        downsample_param = F.adaptive_avg_pool2d(downsample_param.permute(1,0,2,3), (1,1))
        miner_value = torch.ones_like(downsample_param) * 1e-8
        self.downsample_param = torch.where(downsample_param!=0, downsample_param, miner_value)
    ...
    def visual_analyze(self, modulated_search, xcorr_map, processed_xcorr_map):
        ...
        #for i, xcorr_m in enumerate(processed_xcorr_map.permute(1,0,2,3)):
            #print(xcorr_m.shape)
        #    visualizer.merge_tensor_image_and_single_map(self.im_patches[0], xcorr_m, 144, path=visual_root_path,
        #                                name='{:04d}_{:02d}_merge_ppxcorr'.format(self.frame_num, i))

        #visualizer.merge_tensor_image_and_single_map(self.im_patches[0], processed_xcorr_map.sum(dim=1), 144, path=visual_root_path,
        #                            name='{:04d}_sum_merge_ppxcorr'.format(self.frame_num))

        #visualizer.merge_tensor_image_and_single_map(self.im_patches[0], xcorr_map.sum(dim=1), 144, path=visual_root_path,
        #                            name='{:04d}_sum_merge_xcorr'.format(self.frame_num))
    ...
    def _convert_score(self, score):
        ...
        return score
    def _convert_bbox(self, bbox_offsets):
        """
        args:
            bbox_offsets (Tensor) - [top, left, bottom, right]
        returns:
            detections (Tensor) - [4, height, width]
        """
        bbox_offsets = bbox_offsets.cpu().squeeze()
        detections = torch.stack([
            self.points[1,:,:] - bbox_offsets[1,:,:],
            self.points[0,:,:] - bbox_offsets[0,:,:],
            self.points[1,:,:] + bbox_offsets[3,:,:],
            self.points[0,:,:] + bbox_offsets[2,:,:]
        ], dim=0)# [y1, x1, y2, x2]
        return detections

    def get_subsearch_online_score(self, online_score, rescale_size, subseach_window):
        """
        args:
            online_score (Tensor) - Dims (heigh, width)
            subseach_window (Tensor) - [x1, y1, w, h]
        """
        online_score = F.interpolate(online_score.unsqueeze(0).unsqueeze(0), rescale_size, mode='bilinear')
        subseach_window = torch.cat((subseach_window[:,:2], subseach_window[:,:2]+subseach_window[:,2:]-1), dim=-1)
        roi = torch.cat((torch.zeros(1,1, device=subseach_window.device), subseach_window), dim=-1)

        subsearch_online_score = prroi_pool2d(online_score, roi, self.subsearch_feat_sz[0].item(),
                                self.subsearch_feat_sz[1].item(), 1./self.regnet_stride)

        return subsearch_online_score.squeeze()

    def target_state_estimate(self, skfeat, sample_pos, sample_scale, online_score, scale_ind, update_scale=True):
        """
        args:
            skfeat (Tensor, gpu)
        ...
        if __visualize__:
            ...
            bbox_offsets, modulated_search, xcorr_map, processed_xcorr_map = self.net.state_estimator.track(self.template, se_feat, subsearch_window_coord.to(skfeat.device),
                                *(self.subsearch_feat_sz.to(torch.int).numpy().tolist()))
            self.visual_analyze(modulated_search, xcorr_map, processed_xcorr_map)
        ...
        offline_score = self._convert_score(output_cls)

        if self.params.get('fuse_online_offline', False):
            fuse_online_weight = self.params.get('fuse_online_weight', 0.5)
            subsearch_online_score = self.get_subsearch_online_score(online_score, skfeat.shape[-2:],
                    subsearch_window_coord.to(skfeat.device)).cpu()
            fuse_score = subsearch_online_score * fuse_online_weight + offline_score * (1 - fuse_online_weight)

            detections = torch.cat([detections[0:2,:], detections[2:,:]-detections[0:2,:]+1], dim=0)#[y1, x1, h, w]
            detections = detections.permute(1,2,0).contiguous().view(-1, 4).permute(1,0).numpy()
            fuse_score = fuse_score.view(-1).numpy()
            if self.params.get('perform_post_process', False):
                ...
                # scale penalty
                s_c = change(sz(detections[2, :]*sample_scale.item(), detections[3, :]*sample_scale.item()) /
                         (sz(self.target_sz[0].item(), self.target_sz[1].item())))
                # aspect ratio penalty
                r_c = change((self.target_sz[0].item()/self.target_sz[1].item()) /
                ...
                best_idx = np.argmax(fuse_score)
                lr = 1.0
            target_state_subwindow = torch.tensor(detections[:, best_idx])#[y1, x1, h, w]
            target_state = torch.cat([target_state_subwindow[0:2]+window_lt, target_state_subwindow[2:4]], dim=0)

            new_pos = target_state[0:2] + (target_state[2:4]-1)/2
            new_pos = (new_pos - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos

            new_target_sz = target_state[2:4] * sample_scale

            new_target_sz = new_target_sz * lr + self.target_sz * (1-lr)
            ...
            target_state = torch.cat([target_state_subwindow[0:2]+window_lt, target_state_subwindow[2:4]+window_lt], dim=0)

            new_pos = (target_state[0:2] + target_state[2:4])/2
            new_pos = (new_pos - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos
    ...
    def _bbox_clip(self, new_pos, new_target_sz):

        new_target_sz[0].clamp_(min=self.target_sz[0].item()*0.1, max=self.target_sz[0].item()*10.0)
        new_target_sz[1].clamp_(min=self.target_sz[1].item()*0.1, max=self.target_sz[1].item()*10.0)

        #　constain the target in the image
        new_target_tl = new_pos - (new_target_sz - 1) / 2
        new_target_br = new_pos + (new_target_sz - 1) / 2

        new_target_tl[0].clamp_(min=0, max=self.image_sz[0].item()-1)
        new_target_tl[1].clamp_(min=0, max=self.image_sz[1].item()-1)

        new_target_br[0].clamp_(min=0, max=self.image_sz[0].item()-1)
        new_target_br[1].clamp_(min=0, max=self.image_sz[1].item()-1)

        new_pos = (new_target_br + new_target_tl) / 2
        new_target_sz = (new_target_br - new_target_tl + 1).clamp(min=1)

        return new_pos, new_target_sz

    def compute_neighbour(self, center, stride, feat_sz):
        """All input in original image coordinatesl
        args:
            center (Tensor) - [y, x] in image
            feat_sz (Tensor) - [height, width] of the subsearch_feat
        returns
            neighbour (Tensor) - [[y,x]] in feature map
        """

        half_stride = math.floor(stride/2)
        neighbour_in_image = torch.stack([
            center + torch.Tensor([-1*half_stride, -1*half_stride], device=center.device),# tl_neighbour
            center + torch.Tensor([half_stride, -1*half_stride], device=center.device),# bl_neighbour
            center + torch.Tensor([-1*half_stride, half_stride], device=center.device),# tr_neighbour
            center + torch.Tensor([half_stride, half_stride], device=center.device),# br_neighbour
        ], dim=0)

        neighbour_in_image = torch.stack([torch.clamp(neighbour_in_image[:,0], 0, (feat_sz[0]).item()*stride-1),
                    torch.clamp(neighbour_in_image[:,1], 0, (feat_sz[1]).item()*stride-1)], dim=-1)

        neighbour_in_feat = torch.floor(neighbour_in_image / stride)
        neighbour_in_image = neighbour_in_feat * stride + half_stride
        distance = torch.sqrt(torch.pow(neighbour_in_image - center.unsqueeze(0), 2).sum(dim=1))/stride
        weight = torch.exp(-1*distance)/(torch.exp(-1*distance).sum())
        return neighbour_in_feat.to(torch.int), weight
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        predicted_box = output_boxes[inds, :].mean(0)# shape = [4]
        predicted_iou = output_iou.view(-1, 1)[inds, :].mean(0)

        # Get new position and size
        new_pos = predicted_box[:2] + predicted_box[2:] / 2# [xc, yc] in search window
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos# [yc, xc] in testing frame
        new_target_sz = predicted_box[2:].flip((0,)) * sample_scale# [h ,w]
        new_scale = torch.sqrt(new_target_sz.prod() / self.base_target_sz.prod())
