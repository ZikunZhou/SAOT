# configuration corresponding to the final result for the lasot dataset reported in the paper.
...
def run(settings):
    ...
    kp_opt.seintegrate_settings.use_psr = False
    kp_opt.seintegrate_settings.mainlobe_window_radius=2
    kp_opt.seintegrate_settings.use_psrw = True
    ...
    kp_opt.seintegrate_settings.supress_sigma_factor = 1.50#可以改成一个可学习参数
    kp_opt.seintegrate_settings.learn_saliency_scale_factor = True
    kp_opt.seintegrate_settings.saliency_scale_factor = 0.25#可以改成一个可学习参数
    ...
    kp_opt.sedetector_settings.detach_cls = True# 这个最后模型都确定了再分析
    ...
    dataset_train = sampler.DiMPSampler(#[lasot_train, got10k_train, trackingnet_train, coco_train], [0.25,1,1,1],
                                        [lasot_train],[1],
                                        samples_per_epoch=26000, max_gap=150, num_test_frames=3, num_train_frames=3,
    ...
    dataset_val = sampler.DiMPSampler([got10k_val], [1], samples_per_epoch=5000, max_gap=150,
