...
class SaliencyEvaluator_PSR(nn.Module):
    def __init__(self, mainlobe_window_radius=2, detach_saliency=False, use_std_dev=True):
    ...
    def forward(self, cost_volume, peak_coords, mesh):
        ...
        if self.use_std_dev:
            var_sidelobe = pow(pow((cost_volume_flatten - mean_sidelobe) * weights, 2).sum(dim=-1, keepdim=True) / (num_sidelobe_points - 1) + 1e-16, 0.5)
        else:
        ...
        psr = ((peaks - mean_sidelobe) / (var_sidelobe+1e-16)).view(batch, channel)
