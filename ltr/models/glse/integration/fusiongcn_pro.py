...
class FusionGCN(nn.Module):
    def __init__(self, gcn_in_channels, gcn_hidden, gcn_out_channels, graph_sizes, phi_in_channels,
                    phi_hidden_channels, cat_feat_xcorr, gcn_process_feat_xcorr,
                    use_high_order_poly=False, order_num=2, use_4neigh=False):
        """
        args:
        ...
        self.cat_feat_xcorr = cat_feat_xcorr
        self.gcn_process_feat_xcorr = gcn_process_feat_xcorr
        self.use_4neigh = use_4neigh
        ...
        if not use_4neigh:
            ...
            self.coords_pair_4neigh_dict = OrderedDict()
            for graph_size in self.graph_sizes:
                self.coords_pair_4neigh_dict[graph_size] = self.gen_4neigh_coords_pair(*graph_size)
        ...
        if self.cat_feat_xcorr or self.gcn_process_feat_xcorr:
    ...
    def forward(self, search_feature, xcorr_map, saliency, key_coords, graph_size):
        ...
        if not self.use_4neigh:
            coords_pair = self.gen_coords_pair(coords_pair_kpoint, self.coords_pair_8neigh_dict[graph_size], search_feature.device)
        else:
            coords_pair = self.gen_coords_pair(coords_pair_kpoint, self.coords_pair_4neigh_dict[graph_size], search_feature.device)
        ...
        #print('spatial_mask.shape1', spatial_mask.shape)
        spatial_mask = spatial_mask.view(batch, self.gcn_out_channels, height, width)
        #print('spatial_mask.shape2', spatial_mask.shape)
        return spatial_mask
    ...
    def gen_4neigh_coords_pair(self, height, width):#应该对这个邻接矩阵做一个行归一化
        #不同的图对应的这个邻接矩阵应该一致
        row_coord = torch.arange(height)
        column_coord = torch.arange(width)
        row_coords, column_coords = torch.meshgrid(row_coord, column_coord)
        ...
        for i, i_coor in enumerate(coords):
            for j, j_coor in enumerate(coords):
                distance = torch.pow((i_coor - j_coor), 2).sum()
                if distance == 1:
                    coords_pair_4neigh.append([i, j])
        coords_pair_4neigh = torch.tensor(coords_pair_4neigh)
        print(len(coords_pair_4neigh))
        return coords_pair_4neigh

    def gen_kpoint_coords_pair(self, height, width, key_coords):
        """
        key_coords (Tensor) - shape - [num_images*num_sequences, num_keypoints, 2]
        """
        #print(key_coords.shape)
        coords_pair_kpoint = []
