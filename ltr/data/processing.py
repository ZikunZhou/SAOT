...
class DiMPProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor, zoomin_scale, crop_type='replicate',
                 max_scale_change=None, mode='pair', proposal_params=None, label_function_params=None, *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region relative to the target size. Actually 5 in DiMP
            output_sz - An integer, denoting the size to which the search region is resized. The search region is always
                        square.
            center_jitter_factor - A dict containing the amount of jittering to be applied to the target center before
                                    extracting the search region. See _get_jittered_box for how the jittering is done.
            ...
            mode - Either 'pair' or 'sequence'. If mode='sequence', then output has an extra dimension for frames
            proposal_params - Arguments for the proposal generation process. See _generate_proposals for details.
                                Here, the proposals are used for IoU-Net
        ...
        self.center_jitter_factor = center_jitter_factor
        self.scale_jitter_factor = scale_jitter_factor
        self.zoomin_scale = zoomin_scale
        ...
        self.index = 0#for visualization
    ...
    def _get_jittered_box(self, box, mode):
        ...
        returns:
            torch.Tensor - jittered box
        """
        # scale的值越小, 目标在search window中越大, 这里可以限制scale的值, 以免目标超出图像边界, 但是到底有这个必要么
        scale = torch.exp(torch.randn(2) * self.scale_jitter_factor[mode])
        jittered_size = box[2:4] * scale
        max_offset = (jittered_size.prod().sqrt() * torch.tensor(self.center_jitter_factor[mode]).float())
        jittered_center = box[0:2] + 0.5 * box[2:4] + max_offset * (torch.rand(2) - 0.5)
    ...
    def zoomin(self, input_sz, bboxes, output_sz):
        """
        args:
            bboxes (list[Tensor]) - coordinate_defination [x1, y1, w, h]
        returns:
            windows (list[Tensor]) - [x1, y1, w, h]
            bboxes_in_windows (list[Tensor]) - [x1, y1, w, h]
        """
        if isinstance(input_sz, (float, int)):
            input_sz = (input_sz, input_sz)
        input_sz = torch.Tensor(input_sz)
        ...
        windows, bboxes_in_windows = [], []
        for bbox in bboxes:
            box_center = bbox[0:2] + 0.5 * bbox[2:4]
            box_size = bbox[2:4]
            rand_center = torch.round(box_center + (output_sz-box_size) * (torch.rand(2)-0.5))
            rand_window = torch.cat([rand_center - output_sz/2, rand_center + output_sz/2-1], dim=0)
            x1, y1, x2, y2 = rand_window
            shift_x = max(0, -x1) + min(0, input_sz[0]-1-x2)
            x1 += shift_x
            x2 += shift_x
            shift_y = max(0, -y1) + min(0, input_sz[1]-1-y2)
            y1 += shift_y
            y2 += shift_y

            bbox_x1, bbox_y1, bbox_x2, bbox_y2 = torch.cat([bbox[0:2], bbox[0:2]+bbox[2:4]-1], dim=0)
            bbox_x1 = max(0, bbox_x1 - x1)
            bbox_y1 = max(0, bbox_y1 - y1)
            bbox_x2 = min(output_sz[0]-1, bbox_x2 - x1)
            bbox_y2 = min(output_sz[0]-1, bbox_y2 - y1)

            window = torch.tensor([x1, y1, x2-x1+1, y2-y1+1])
            bbox_in_window = torch.tensor([bbox_x1, bbox_y1, bbox_x2-bbox_x1+1, bbox_y2-bbox_y1+1])
            windows.append(window)
            bboxes_in_windows.append(bbox_in_window)

        return windows, bboxes_in_windows
    ...
    def __call__(self, data: TensorDict):
        ...
        Caution: the coordinate defination of train_anno, test_anno and test_proposals is [x1, y1, w, h]
        """

        if self.transform['joint'] is not None:# not None
        ...
        for s in ['train', 'test']:
            ...
            # Add a uniform noise to the center pos
            jittered_anno = [self._get_jittered_box(a, s) for a in data[s + '_anno']]
            # jittered_anno就起到对目标进行尺度变换,平移操作的作用
            ...
            if s == 'test':
                data[s+'_win'], data[s+'_bb_inwin'] = self.zoomin(self.output_sz, data[s + '_anno'], self.output_sz*self.zoomin_scale)
                data[s+'_win_img'] = [img[:, int(roi[1]):int(roi[1]+roi[3]), int(roi[0]):int(roi[0]+roi[2])] for (img, roi) in zip(data[s + '_images'], data[s+'_win'])]

        #a = Visualizer()#the following commented codes are only used for visualization
        #for i in range(len(data['test_images'])):
        #    a.visualize_normed_image_with_box(data['test_win_img'][i], data['test_bb_inwin'][i],
        #                    output_size=(288, 288), path='/home/zikun/work/tracking/KPT/process/', name='image{:0>3d}_{:0>3d}'.format(self.index, i))
        #    a.visualize_normed_image_with_box(data['test_images'][i], data['test_anno'][i],
        #                    output_size=(288, 288), path='/home/zikun/work/tracking/KPT/process', name='image_ori{:0>3d}_{:0>3d}'.format(self.index, i))
        #self.index += 1
        # Generate proposals, 只是生成了一系列bounding box, 训练的时候这些bounding box 就是IoU-Net testing branch 中的candidate box
...
class KLDiMPProcessing(BaseProcessing):
    ...
    def _get_jittered_box(self, box, mode):
        """ Jitter the input box
        args:
            box - input bounding box [x1, y1, w, h]
            mode - string 'train' or 'test' indicating train or test data
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:# actually not none
            data['train_images'], data['train_anno'] = self.transform['joint'](image=data['train_images'], bbox=data['train_anno'])
            data['test_images'], data['test_anno'] = self.transform['joint'](image=data['test_images'], bbox=data['test_anno'], new_roll=False)
