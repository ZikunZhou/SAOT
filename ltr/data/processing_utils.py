...
def sample_target_adaptive(im, target_bb, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    shift_x = max(0, -x1) + min(0, im_w - x2)#要裁剪的区域是否超过图像左右边界
    x1 += shift_x
    x2 += shift_x

    shift_y = max(0, -y1) + min(0, im_h - y2)#要裁剪的区域是否超过图像上下边界
    y1 += shift_y
    y2 += shift_y

    out_x = (max(0, -x1) + max(0, x2 - im_w)) // 2#要裁剪的区域超出图像边界的长度
...
def exponent_label_function(target_bb, threshold, image_sz, sigma_factor):
    """　Construct　exponent label function
    """
    if isinstance(image_sz, (float, int)):
        image_sz = (image_sz, image_sz)
    num_images = target_bb.shape[0]
    target_center = target_bb[:, 0:2] + 0.5 * target_bb[:, 2:4]
    target_size = target_bb[:,2:4]
    target_bb_xyxy = torch.cat((target_center-threshold*target_size, target_center+threshold*target_size), dim=-1)
    target_center = target_center.unsqueeze(-1).unsqueeze(-1)
    target_size = target_size.unsqueeze(-1).unsqueeze(-1)

    l, t, r, b = target_bb_xyxy.permute(1,0).unsqueeze(-1).unsqueeze(-1)

    y_coordinates, x_coordinates = torch.meshgrid(torch.arange(image_sz[0]),
                                   torch.arange(image_sz[1]))
    points = torch.stack([y_coordinates.to(torch.float), x_coordinates.to(torch.float)]).unsqueeze(0).expand(num_images, -1, -1, -1)#[2, 288, 288]

    windows = ((points[:,0,:,:]>t) * (points[:,0,:,:]<b) * (points[:,1,:,:]>l) * (points[:,1,:,:]<r)).to(torch.float)

    sigma = sigma_factor * target_size

    distance = torch.pow(points[:,0,:,:]-target_center[:,1],2)/(2*torch.pow(sigma[:,1], 2))+\
                    torch.pow(points[:,1,:,:]-target_center[:,0],2)/(2*torch.pow(sigma[:,0],2))

    labels = torch.exp(-1*distance)
    labels = labels /(labels.view(num_images, -1).max(dim=-1, keepdim=True)[0].unsqueeze(-1))
    labels = labels * windows

    #a = Visualizer()
    #for i in range(num_images):
    #    a.visualize_single_map(windows[i].unsqueeze(0), output_size=(288, 288),
    #                path='/home/zikun/work/tracking/KPT', name='window{:d}'.format(i))
    #    a.visualize_single_map(labels[i].unsqueeze(0), output_size=(288, 288),
    #                path='/home/zikun/work/tracking/KPT', name='label{:d}'.format(i))
    #    a.visualize_single_map(labels0[i].unsqueeze(0), output_size=(288, 288),
    #                path='/home/zikun/work/tracking/KPT', name='label0{:d}'.format(i))
