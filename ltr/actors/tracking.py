...
class DiMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        target_scores, bboxes, cls = self.net(train_imgs=data['train_images'],
                                                   ...
                                                   test_win=data['test_win'],
                                                   test_bb_inwin=data['test_bb_inwin'])
        ...
        iou_loss, cls_loss = self.objective['box'](bboxes, cls, data['test_bb_inwin'])
        iou_loss = self.loss_weight['box'] * iou_loss
        if cls is not None:
            cls_loss = self.loss_weight['cls'] * cls_loss
        ...
        if cls is not None:
            # Total loss
            loss = iou_loss + cls_loss + loss_target_classifier + loss_test_init_clf + loss_test_iter_clf
            # Log stats
            stats = {'Loss/total': loss.item(),
                    'Loss/iou_loss': iou_loss.item(),
                    'Loss/cls_loss': cls_loss.item(),
                    'Loss/target_clf': loss_target_classifier.item()}
        else:
            # Total loss
            loss = iou_loss + loss_target_classifier + loss_test_init_clf + loss_test_iter_clf
            # Log stats
            stats = {'Loss/total': loss.item(),
                    'Loss/iou_loss': iou_loss.item(),
                    'Loss/target_clf': loss_target_classifier.item()}
